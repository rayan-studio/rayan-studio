<div align="center">

# Rayan El Manssouri
### Full-Stack Developer

Développeur focalisé sur les applications web, les outils logiciels et les projets full-stack concrets.

[![GitHub](https://img.shields.io/badge/GitHub-Rayan--Walnut-111827?style=flat&logo=github&logoColor=white)](https://github.com/Rayan-Walnut)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Rayan%20El%20Manssouri-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/rayan-el-manssouri-1972143a1/)
[![Email](https://img.shields.io/badge/Email-rayanelmanssouri%40gmail.com-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:rayanelmanssouri@gmail.com)

</div>

---

## A propos

Etudiant en BTS SIO option SLAM, avec un parcours oriente vers le developpement full-stack et les outils logiciels.

Je travaille principalement avec React, C#, JavaScript, TypeScript et C++, avec un interet particulier pour les projets concrets, l'automatisation et les interfaces propres.

| | |
|---|---|
| Localisation | Le Pontet, France |
| Formation actuelle | BTS SIO SLAM — Lycee CCI du Gard (2024 - 2026) |
| Suite de parcours | Bachelor Developpement Fullstack en alternance — CODA Avignon (2026 - 2027) |
| Langues | Francais (natif), Anglais (B1 / B2) |

---

## Stack technique

<div align="center">
  <img src="https://skillicons.dev/icons?i=react,ts,js,cs,cpp,html,css,php,powershell,git,github,wordpress&perline=6" alt="Tech stack" />
</div>

---

## Experience

| Structure | Mission |
|---|---|
| Direction de l'Innovation et des Systemes d'Information — Nimes | Developpement d'un outil CLI en C# pour l'envoi de SMS, et creation d'un site web de consultation de logs TrueNAS |
| Inersio — Sorgues | Travail sur le style d'un site administratif en JavaScript |
| Geoitech Trust — Maroc | Decouverte et utilisation de WordPress |
| ITSO Technologies SAS — Sorgues | Decouverte des bases de la programmation en PHP |

---

## Statistiques GitHub

<div align="center">
  <img width="49%" src="https://github-readme-stats.vercel.app/api?username=Rayan-Walnut&show_icons=true&theme=tokyonight&hide_border=true&count_private=true&bg_color=0d1117&title_color=58a6ff&icon_color=58a6ff&text_color=c9d1d9" alt="GitHub stats" />
  <img width="49%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Rayan-Walnut&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=58a6ff&text_color=c9d1d9" alt="Top languages" />
</div>

<div align="center">
  <img width="100%" src="https://github-readme-streak-stats.herokuapp.com/?user=Rayan-Walnut&theme=tokyonight&hide_border=true&background=0d1117&ring=58a6ff&fire=58a6ff&currStreakLabel=58a6ff" alt="GitHub streak" />
</div>

<div align="center">
  <img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=Rayan-Walnut&theme=tokyo-night&hide_border=true&bg_color=0d1117&color=58a6ff&line=58a6ff&point=c9d1d9" alt="Activity graph" />
</div>

---

## Projets

| Projet | Description |
|---|---|
| [Nebula](https://astracode.dev) | Projet mis en avant sur astracode.dev, oriente outil de developpement et univers logiciel |
| [Quantum](https://github.com/Rayan-Walnut/Quantum) | Navigateur web personnel et experimentation logicielle |
| [mercurev2](https://github.com/Rayan-Walnut/mercurev2) | Projet de messagerie moderne en TypeScript |
| [Lumina](https://github.com/Rayan-Walnut/Lumina) | Experimentation d'interface et de composants en C++ |
| [Pokemon TCG](https://github.com/Rayan-Walnut/Pok-mon-TCG) | Application TypeScript autour de l'univers Pokemon TCG |
| [services](https://github.com/Rayan-Walnut/services) | Scripts PowerShell pour optimisation et automatisation |
| [Aborescence](https://github.com/Rayan-Walnut/Aborescence) | Utilitaire de copie de structure de dossiers |

---

## Focus actuel

- Renforcer mon niveau en developpement full-stack
- Construire des projets plus propres techniquement et plus presentables
- Ameliorer l'architecture, la qualite de code et la logique produit

---

## Contact

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-Rayan--Walnut-111827?style=flat&logo=github&logoColor=white)](https://github.com/Rayan-Walnut)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Rayan%20El%20Manssouri-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/rayan-el-manssouri-1972143a1/)
[![Email](https://img.shields.io/badge/Email-rayanelmanssouri%40gmail.com-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:rayanelmanssouri@gmail.com)

</div>

<!-- Metrics setup : ajouter un secret de depot nomme METRICS_TOKEN, puis lancer le workflow Metrics une fois. -->